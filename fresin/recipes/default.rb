...
unless node[:resin][:mysql].nil?
  mysql_version = node[:resin][:mysql][:version]
  remote_file 'install mysql jdbc' do
    path "#{resin_alias}/ext-lib/mysql-connector-java-#{mysql_version}.jar"
    source "#{maven_search_path}mysql/mysql-connector-java/#{mysql_version}/mysql-connector-java-#{mysql_version}.jar"
  end
end
